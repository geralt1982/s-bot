...
def format_seconds_to_human_readable(total_seconds: int) -> str:
    ...
    if days > 0:
        parts.append(f"{days} день" + ("я" if days == 1 else "ей" if days < 5 else "ей"))
    if hours > 0:
        parts.append(f"{hours} час" + ("" if hours == 1 else "а" if hours < 5 else "ов"))
    if minutes > 0:
        parts.append(f"{minutes} минут" + ("а" if minutes == 1 else "ы" if minutes < 5 else ""))

    if seconds > 0 or (not parts and total_seconds == 0):
        parts.append(f"{seconds} секунд" + ("а" if seconds == 1 else "ы" if seconds < 5 else ""))
